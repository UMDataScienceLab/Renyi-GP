...
logL=function(H,fn)
{
  ...
  A <- diag(H[2*n+2]^2, dim(fuf)[1]) + (1-H[6]) * r_cff
  ch <- chol(A + diag(0.2, ncol(A) ), pivot=TRUE, tol=1e-16)
  ...
  B2 <-  diag(1, ncol(r_cuu)) + (1-2*H[6]) * mBCG_noT(r_cuu, t(r_cfu), maxiter = MAT_ITER)%*%RHS[,2:(ncol(RHS))]
