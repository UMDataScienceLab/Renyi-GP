...
logL=function(H,fn)
{
  ...
  B <- H[2+dim+1] * fuf + diag(H[1]^2, dim(fuf)[1]) + (1-H[2+dim+1]) * r_cff # calculate the covariance of alpha-ELBO objective
  
  RHS <- cbind(y, r_cfu)
  
  B_inv_y <- mBCG_noT(B, RHS, maxiter = MAT_ITER) # run conjugate method for efficient matrix product
  
  A <- diag(H[1]^2, dim(fuf)[1]) + (1-H[2+dim+1]) * r_cff + H[2+dim+1] * fuf # calculate matrix \xi in Eq. (6) defined in Appendix C
  ch <- chol(A + diag(0.05, ncol(A) ), pivot=TRUE, tol=1e-16) # cholesky decomposition 
  ...
  B2 <-  diag(1, ncol(r_cuu)) + (1-2*H[2+dim+1]) * mBCG_noT(r_cuu, t(r_cfu), maxiter = MAT_ITER)%*%B_inv_y[,2:(ncol(RHS))] # calculate C_x in Appendix C
